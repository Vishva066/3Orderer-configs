...
export ORDERER1_ADMIN_TLS_SIGN_CERT=${PWD}/organizations/ordererOrganizations/auto.com/orderers/orderer1.auto.com/tls/server.crt

export ORDERER1_ADMIN_TLS_PRIVATE_KEY=${PWD}/organizations/ordererOrganizations/auto.com/orderers/orderer1.auto.com/tls/server.key

export ORDERER2_ADMIN_TLS_SIGN_CERT=${PWD}/organizations/ordererOrganizations/auto.com/orderers/orderer2.auto.com/tls/server.crt

export ORDERER2_ADMIN_TLS_PRIVATE_KEY=${PWD}/organizations/ordererOrganizations/auto.com/orderers/orderer2.auto.com/tls/server.key

export ORDERER3_ADMIN_TLS_SIGN_CERT=${PWD}/organizations/ordererOrganizations/auto.com/orderers/orderer3.auto.com/tls/server.crt

export ORDERER3_ADMIN_TLS_PRIVATE_KEY=${PWD}/organizations/ordererOrganizations/auto.com/orderers/orderer3.auto.com/tls/server.key


osnadmin channel join --channelID $CHANNEL_NAME --config-block ${PWD}/channel-artifacts/$CHANNEL_NAME.block -o localhost:7053 --ca-file $ORDERER_CA --client-cert $ORDERER1_ADMIN_TLS_SIGN_CERT --client-key $ORDERER1_ADMIN_TLS_PRIVATE_KEY
sleep 2
osnadmin channel join --channelID $CHANNEL_NAME --config-block ${PWD}/channel-artifacts/$CHANNEL_NAME.block -o localhost:8053 --ca-file $ORDERER_CA --client-cert $ORDERER2_ADMIN_TLS_SIGN_CERT --client-key $ORDERER2_ADMIN_TLS_PRIVATE_KEY
sleep 2
osnadmin channel join --channelID $CHANNEL_NAME --config-block ${PWD}/channel-artifacts/$CHANNEL_NAME.block -o localhost:9053 --ca-file $ORDERER_CA --client-cert $ORDERER3_ADMIN_TLS_SIGN_CERT --client-key $ORDERER3_ADMIN_TLS_PRIVATE_KEY
sleep 2

osnadmin channel list -o localhost:7053 --ca-file $ORDERER_CA --client-cert $ORDERER1_ADMIN_TLS_SIGN_CERT --client-key $ORDERER1_ADMIN_TLS_PRIVATE_KEY
sleep 2
osnadmin channel list -o localhost:8053 --ca-file $ORDERER_CA --client-cert $ORDERER2_ADMIN_TLS_SIGN_CERT --client-key $ORDERER2_ADMIN_TLS_PRIVATE_KEY
sleep 2
osnadmin channel list -o localhost:9053 --ca-file $ORDERER_CA --client-cert $ORDERER3_ADMIN_TLS_SIGN_CERT --client-key $ORDERER3_ADMIN_TLS_PRIVATE_KEY
sleep 2
...
peer channel fetch config ${PWD}/channel-artifacts/config_block.pb -o localhost:7050 --ordererTLSHostnameOverride orderer1.auto.com -c $CHANNEL_NAME --tls --cafile $ORDERER_CA
sleep 1
...
peer channel update -f ${PWD}/channel-artifacts/config_update_in_envelope.pb -c $CHANNEL_NAME -o localhost:7050  --ordererTLSHostnameOverride orderer1.auto.com --tls --cafile $ORDERER_CA
sleep 1
...
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer1.auto.com --channelID $CHANNEL_NAME --name KBA-Automobile --version 1.0 --collections-config ../Chaincode/collection-automobile.json --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
sleep 2
...
peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer1.auto.com --channelID $CHANNEL_NAME --name KBA-Automobile --version 1.0 --sequence 1 --collections-config ../Chaincode/collection-automobile.json --tls --cafile $ORDERER_CA --peerAddresses localhost:7051 --tlsRootCertFiles $MANUFACTURER_PEER_TLSROOTCERT --peerAddresses localhost:9051 --tlsRootCertFiles $DEALER_PEER_TLSROOTCERT --peerAddresses localhost:11051 --tlsRootCertFiles $MVD_PEER_TLSROOTCERT
sleep 1
